  ...
  apply: function (name, args, options, callback) {                                                                    // 1460
    ...
    if (!handler) {                                                                                                    // 1484
      ...
      try {                                                                                                            // 1511
        ...
        result = EJSON.clone(result);                                                                                  // 1517
      } catch (e) {                                                                                                    // 1518
        exception = e;                                                                                                 // 1519
      }                                                                                                                // 1520
    }                                                                                                                  // 1521
                                                                                                                       // 1522
    // Return the result in whichever way the caller asked for it. Note that we                                        // 1523
    // do NOT block on the write fence in an analogous way to how the client                                           // 1524
    // blocks on the relevant data being visible, so you are NOT guaranteed that                                       // 1525
    // cursor observe callbacks have fired when your callback is invoked. (We                                          // 1526
    // can change this if there's a real use case.)                                                                    // 1527
    if (callback) {                                                                                                    // 1528
      callback(exception, result);                                                                                     // 1529
      return undefined;                                                                                                // 1530
    }                                                                                                                  // 1531
    if (exception)                                                                                                     // 1532
      throw exception;                                                                                                 // 1533
    return result;                                                                                                     // 1534
  },                                                                                                                   // 1535
                                                                                                                       // 1536
  _urlForSession: function (sessionId) {                                                                               // 1537
    var self = this;                                                                                                   // 1538
    var session = self.sessions[sessionId];                                                                            // 1539
    if (session)                                                                                                       // 1540
      return session._socketUrl;                                                                                       // 1541
    else                                                                                                               // 1542
      return null;                                                                                                     // 1543
  }                                                                                                                    // 1544
});                                                                                                                    // 1545
                                                                                                                       // 1546
var calculateVersion = function (clientSupportedVersions,                                                              // 1547
                                 serverSupportedVersions) {                                                            // 1548
  var correctVersion = _.find(clientSupportedVersions, function (version) {                                            // 1549
    return _.contains(serverSupportedVersions, version);                                                               // 1550
  });                                                                                                                  // 1551
  if (!correctVersion) {                                                                                               // 1552
    correctVersion = serverSupportedVersions[0];                                                                       // 1553
  }                                                                                                                    // 1554
  return correctVersion;                                                                                               // 1555
};                                                                                                                     // 1556
                                                                                                                       // 1557
LivedataTest.calculateVersion = calculateVersion;                                                                      // 1558
                                                                                                                       // 1559
                                                                                                                       // 1560
// "blind" exceptions other than those that were deliberately thrown to signal                                         // 1561
// errors to the client                                                                                                // 1562
var wrapInternalException = function (exception, context) {                                                            // 1563
  if (!exception || exception instanceof Meteor.Error)                                                                 // 1564
    return exception;                                                                                                  // 1565
                                                                                                                       // 1566
  // tests can set the 'expected' flag on an exception so it won't go to the                                           // 1567
  // server log                                                                                                        // 1568
  if (!exception.expected) {                                                                                           // 1569
    Meteor._debug("Exception " + context, exception.stack);                                                            // 1570
    if (exception.sanitizedError) {                                                                                    // 1571
      Meteor._debug("Sanitized and reported to the client as:", exception.sanitizedError.message);                     // 1572
      Meteor._debug();                                                                                                 // 1573
    }                                                                                                                  // 1574
  }                                                                                                                    // 1575
                                                                                                                       // 1576
  // Did the error contain more details that could have been useful if caught in                                       // 1577
  // server code (or if thrown from non-client-originated code), but also                                              // 1578
  // provided a "sanitized" version with more context than 500 Internal server                                         // 1579
  // error? Use that.                                                                                                  // 1580
  if (exception.sanitizedError) {                                                                                      // 1581
    if (exception.sanitizedError instanceof Meteor.Error)                                                              // 1582
      return exception.sanitizedError;                                                                                 // 1583
    Meteor._debug("Exception " + context + " provides a sanitizedError that " +                                        // 1584
                  "is not a Meteor.Error; ignoring");                                                                  // 1585
  }                                                                                                                    // 1586
                                                                                                                       // 1587
  return new Meteor.Error(500, "Internal server error");                                                               // 1588
};                                                                                                                     // 1589
                                                                                                                       // 1590
                                                                                                                       // 1591
// Audit argument checks, if the audit-argument-checks package exists (it is a                                         // 1592
// weak dependency of this package).                                                                                   // 1593
var maybeAuditArgumentChecks = function (f, context, args, description) {                                              // 1594
  args = args || [];                                                                                                   // 1595
  if (Package['audit-argument-checks']) {                                                                              // 1596
    return Match._failIfArgumentsAreNotAllChecked(                                                                     // 1597
      f, context, args, description);                                                                                  // 1598
  }                                                                                                                    // 1599
  return f.apply(context, args);                                                                                       // 1600
};                                                                                                                     // 1601
                                                                                                                       // 1602
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
